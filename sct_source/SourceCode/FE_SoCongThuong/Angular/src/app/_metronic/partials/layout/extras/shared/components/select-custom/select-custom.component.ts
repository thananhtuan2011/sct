...
import { SelectOptionData } from "./select-custom.interface";
    ...
    ControlValueAccessor
{
  ...
  @Input() data: Array<SelectOptionData>;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (
      changes["dropdownParent"] &&
      changes["dropdownParent"].previousValue !==
        changes["dropdownParent"].currentValue
    ) {
    ...
    if (
      changes["allowClear"] &&
      changes["allowClear"].previousValue !== changes["allowClear"].currentValue
    ) {
      this.renderer.setAttribute(
        this.selector.nativeElement,
        "data-allow-clear",
        this.allowClear.toString()
      );
    }
  }
  ...
  private initPlugin() {
    if (!this.element.select2) {
      if (!this.check) {
        this.check = true;
        console.log(
          "Please add Select2 library (js file) to the project." +
    ...
    if (Array.isArray(newValue)) {
      for (const option of this.selector.nativeElement.options) {
        this.renderer.setProperty(
          option,
          "selected",
          newValue.indexOf(option.value) > -1
        );
      }
    } else {
