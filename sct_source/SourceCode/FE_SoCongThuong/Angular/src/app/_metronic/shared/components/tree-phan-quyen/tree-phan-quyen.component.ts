import { Component, OnInit, ChangeDetectorRef, ChangeDetectionStrategy, Inject, Input, Output, EventEmitter } from '@angular/core';
import { ActivatedRoute, Router } from '@angular/router';
import { FormBuilder, FormGroup, Validators } from '@angular/forms';
import { Observable, forkJoin, from, of, BehaviorSubject } from 'rxjs';
import { EventBusService } from './_services/event-bus.service';
@Component({
	selector: 'app-tree-phan-quyen',
	templateUrl: './tree-phan-quyen.component.html',
	changeDetection: ChangeDetectionStrategy.OnPush
})
export class TreePhanQuyenComponent implements OnInit {
	...
	ngOnInit() {
		...
		this.oldItem = {};
		this.lst_PhanQuyen = this.valuePhanQuyen;
		//load những node đã được selected, check các node được chọn từ db lên theo quyền
		if (this.lst_PhanQuyen) {
			this.checkNodeFromQuyen_DB(this.lst_PhanQuyen[0]);
			this.collapseNode(this.lst_PhanQuyen[0]);
		}

		this.eventBusService.nodeCheckedChange.subscribe(item => {
	...
	onLuuPhanQuyenCay() {
		...
		data.ChiTiet = this.layQuyenDuocChon(this.lst_PhanQuyen[0], data.ChiTiet);
		return data.ChiTiet;
	}

	layQuyenDuocChon(lstSrc: any, lst: any[]) {
		let quyen: any = {};
		if (lstSrc[this.propNameCss].checked) {
	...
	checkNodeFromQuyen_DB(itemSrc: any) {
		...
		//duyệt các node
		if (itemSrc[this.propNameChild] && itemSrc[this.propNameChild].length > 0) {
	...
	phanQuyenBus(item: any) {
		...
		if (item[this.propNameChild]) {
			...
			if (item[this.nameNode] == [this.textFeildRoof]) obj = this.lst_PhanQuyen[0];
			else {
				this.findItemCheck(this.lst_PhanQuyen[0], item);
		...
		{
			this.findParentOfLastNode(this.lst_PhanQuyen[0], item[this.nameNode]);
	...
	checkAllChild(itemA: any) {
		...
		if (itemA[this.propNameChild]) {
			itemA[this.propNameChild].map((item:any, index:any) => {
	...
	checkAllParent(itemA: any, key: any) {
		...
		//let objParent = this.findParentRecursion(this.lst_PhanQuyen[0], key);

		let objParent = Object.assign({}, itemA);

		if (itemA[this.nameNode] != [this.textFeildRoof]) {
			this.findParentRecursion(this.lst_PhanQuyen[0], key);
	...
	collapseNode(itemA: any) {
        ...
        if(itemA[this.nameNode] == 'Tất cả'){
          itemA[this.propNameCss].collapse = true;//mở các node con
        }else{
          itemA[this.propNameCss].collapse = false;//đóng các node con
        }
