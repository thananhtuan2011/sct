...
export class EditTrainingClassManagementModalComponent implements OnInit, OnDestroy {
  ...
  save() {
    ...
    if(this.id){
      model.append('trainingClassManagementId', this.id);
      this.edit(model);
    }else{
      this.create(model);
    }
  ...
  deleteFile(index: number) {
    ...
    if (this.file_documents[index].trainingClassManagementAttachFileId) {
      this.del_file_ids += this.file_documents[index].trainingClassManagementAttachFileId + ','
