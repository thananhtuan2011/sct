...
export class CateRPPCrafttAncolForEconomicPageService extends TableService<any> implements OnDestroy {
  API_URL = `${environment.apiUrl}/CateRPPCrafttAncolForEconomic`;
  ...
  deleteCateRPPCrafttAncolForEconomic(id: string): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
    const url = `${this.API_URL}/delete/${id}`;
    return this.http.put(url, {} ,{ headers: httpHeaders });
  }
  
  deleteCateRPPCrafttAncolForEconomics(ids: any[] = []): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
