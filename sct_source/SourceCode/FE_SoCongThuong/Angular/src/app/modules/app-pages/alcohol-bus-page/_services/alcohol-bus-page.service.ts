...
export class AlcoholBusinessPageService extends TableService<any> implements OnDestroy {
  API_URL = `${environment.apiUrl}/AlcoholBusiness`;
  ...
  deleteAlcoholBusiness(id: string): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
  ...
  public loadListAlcoholWholesaleLicense(){
    const httpHeaders = this._AuthService.getHTTPHeaders();
    return this.http.get<any>(this.API_URL + '/ListAlcoholWholesaleLicense',
      {
        headers: httpHeaders
      });
  }
