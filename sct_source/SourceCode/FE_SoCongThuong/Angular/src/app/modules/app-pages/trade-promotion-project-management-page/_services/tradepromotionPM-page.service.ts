...
export class TradePromotionProjectManagementPageService extends TableService<any> implements OnDestroy {
  API_URL = `${environment.apiUrl}/TradePromotionProjectManagement`;
  ...
  deleteTradePromotionProjectManagement(id: string): Observable<any> {
    // console.log('delete: ',id);
    const httpHeaders = this.auth.getHTTPHeaders();
    const url = `${this.API_URL}/deleteTradePromotionProjectManagement/${id}`;
    return this.http.put(url, {} ,{ headers: httpHeaders });
  }
  
  deleteTradePromotionProjectManagements(ids: any[] = []): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
    const body = { districtIds: ids };
    const url = `${this.API_URL}/deleteTradePromotionProjectManagements`;
  ...
  public loadCurrencyUnit(){
    const httpHeaders = this._AuthService.getHTTPHeaders();
    return this.http.get<any>(this.API_URL + '/loadCurrencyUnit',
      {
        headers: httpHeaders
      });
  }
