...
const EMPTY_CUSTOM: ConsumerServiceRevenueModel = {
  id: '',
  consumerServiceRevenueId: '00000000-0000-0000-0000-000000000000',
...
export class InfoConsumerServiceRevenueModalComponent implements OnInit, OnDestroy {
  @Input() id: any;
  isLoading$: any;
  infoData: ConsumerServiceRevenueModel;
  formGroup: FormGroup;
  ...
  constructor(
    private ConsumerServiceRevenueService: ConsumerServiceRevenuePageService,
    private fb: FormBuilder, public modal: NgbActiveModal,
    private http: HttpClient,
  ) { }

  ngOnInit(): void {
    this.isLoading$ = this.ConsumerServiceRevenueService.isLoading$;
    this.loadUser();
    this.options = {
      theme: 'bootstrap5',
  ...
  loadDetail() {
    const sb = this.ConsumerServiceRevenueService.getItemById(this.id).pipe(
      first(),
      ...
      this.infoData.confirmTime = this.convert_date_string(res.data.confirmTime);
      this.infoData.createTime = this.convert_date_string(res.data.createTime);
  ...
  loadForm() {
    this.formGroup = this.fb.group({
      ConsumerServiceRevenueCode: [this.infoData.consumerServiceRevenueCode, Validators.compose([Validators.required])],
      CheckName: [this.infoData.checkUserId],
      ConfirmName: [this.infoData.confirmUserId],
      CreateName: [this.infoData.createUserId],
      NgayDuyet: [this.infoData.confirmTime, Validators.required],
      NgayTao: [this.infoData.createTime],
      ReportMonth: [this.infoData.reportMonth, Validators.required],
    });
    this.show = true;
  }

  loadUser() {
    this.ConsumerServiceRevenueService.loaduser().subscribe(res => {
      const data = [{
  ...
  exportFile() {
    ...
    const fileName = "Quanlydoanhthudichvutieudung_" + timeString + ".xlsx"
    Swal.fire({
      icon: 'info',
    ...
    this.http.post(`${environment.apiUrl}/ConsumerServiceRevenue/ExportExcel/${this.id}`, {},
    {
      responseType: 'blob',
    }).pipe(
