...
export class RecordsManagerPageService extends TableService<any> implements OnDestroy {
  API_URL = `${environment.apiUrl}/RecordsManager`;
  ...
  getAllRecordsFianacePlan(): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
    const url = `${environment.apiUrl}/RecordsFinancePlan/findAll`;
