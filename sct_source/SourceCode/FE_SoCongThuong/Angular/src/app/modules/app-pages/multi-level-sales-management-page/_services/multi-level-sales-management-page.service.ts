...
export class MultiLevelSalesManagementPageService extends TableService<any> implements OnDestroy {
  API_URL = `${environment.apiUrl}/MultiLevelSalesManagement`;
  ...
  getBusinessMultiLevel(): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
    const url = `${this.API_URL}/getBusinessMultiLevel`;
