...
export class ParticipateTradePromotionService extends TableService<any> implements OnDestroy {
  API_URL = `${environment.apiUrl}/ParticipateTradePromotion`;
  ...
  getItemByCode(code: any): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
    return this.http.get<any>(this.API_URL + `/getItemByCode/${code}`,
      {
        headers: httpHeaders
      });
  }
