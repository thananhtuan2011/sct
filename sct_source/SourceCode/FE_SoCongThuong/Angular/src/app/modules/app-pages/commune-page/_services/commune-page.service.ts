...
export class CommunePageService extends TableService<any> implements OnDestroy {
  API_URL = `${environment.apiUrl}/Commune`;
  ...
  deleteCommune(id: any): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
    const url = `${this.API_URL}/deleteCommune/${id}`;
    return this.http.put(url, {} ,{ headers: httpHeaders });
  }
  
  deleteCommunes(ids: any[] = []): Observable<any> {
    const httpHeaders = this.auth.getHTTPHeaders();
    const body = { districtIds: ids };
    const url = `${this.API_URL}/deleteCommunes`;
