...
import { EditMarketDevelopPlanModalComponent } from './components/edit-market-develop-plan-modal/edit-modal.component';
...
export class TableMarketDevelopPlanPageComponent implements OnInit,
  OnDestroy,
  ICreateAction,
  IEditAction,
  IDeleteAction,
  ...
  constructor(
    ...
    public pageService: MarketDevelopPlanPageService,
  ...
  filterForm() {
    ...
    this.subscriptions.push(
      this.filterGroup.controls.Stage.valueChanges.subscribe(() => this.filter())
    );
  }
  ...
  filter() {
    ...
    const stage = this.filterGroup.controls['Stage'].value;
    ...
    if (stage && stage != "00000000-0000-0000-0000-000000000000") {
      filter['Stage'] = stage;
    }
  ...
  resetFilter() {
    ...
    this.filterGroup.controls.Stage.reset("00000000-0000-0000-0000-000000000000");
    this.pageService.fetch();
  }
  ...
  edit(item: any) {
    const modalRef = this.modalService.open(EditMarketDevelopPlanModalComponent, { size: 'lg' });
    if (item) {
    ...
    modalRef.componentInstance.typeOfMarketPlanData = this.typeOfMarketPlanData;
    modalRef.componentInstance.rankData = this.rankData;
    modalRef.componentInstance.stageData = this.stageData;
  ...
  exportFile() {
    ...
    const fileName = "QuanLyThongTinQuyHoachPhatTrienCho_" + timeString + ".xlsx"
    ...
    this.http.post(`${environment.apiUrl}/MarketDevelopPlan/Export`, query, {
      responseType: 'blob',
    }).pipe(
  ...
  loadRank(){
    this.subscriptions.push(this.commonService.GetConfig('LEVEL_MARKET').subscribe((res: any) => {
      const data = [
        {
          id: '00000000-0000-0000-0000-000000000000',
          text: '-- Chọn --'
  ...
  loadTypeOfMarketPlanData(){
    this.subscriptions.push(this.commonService.GetConfig('TYPE_OF_MARKET_PLAN').subscribe((res: any) => {
      const data = [
        {
          id: '00000000-0000-0000-0000-000000000000',
          text: '-- Chọn --'
      ...
      this.typeOfMarketPlanData = data;
    }))
  }
  
  loadStage() {
    const sb = this.commonService.loadStage().subscribe((res: any) => {
      const data = [
