...
namespace API_SoCongThuong.Controllers
{
    ...
    [ApiController]
    public class IndustrialManagementTargetController : ControllerBase
    {
        private IndustrialManagementTargetRepo _repo;
        private IConfiguration _configuration;
        private readonly ILogger<AsyncLogger> _logger;
        public SoHoa_SoCongThuongContext _context;
        private AsyncLogger _asyncLogger;
        public IndustrialManagementTargetController(SoHoa_SoCongThuongContext context, IConfiguration configuration, ILogger<AsyncLogger> logger)
        {
            _repo = new IndustrialManagementTargetRepo(context);
            _logger = logger;
            _context = context;
        ...
        public IActionResult ListItems_New([FromBody] QueryRequestBody query)//query truyền lên
        {
            ...
            BaseModels<ReponseModel> model = new BaseModels<ReponseModel>();
            ...
            try
            {
                ...
                Func<ReponseModel, object> _orderByExpression = x => x.Name; //Khởi tạo mặc định sắp xếp dữ liệu
                Dictionary<string, Func<ReponseModel, object>> _sortableFields = new Dictionary<string, Func<ReponseModel, object>>   //Khởi tạo các trường để sắp xếp
                    {
                    { "Name", x => x.Name },
                    { "Unit", x => x.Unit },
                ...
                //     });
                IQueryable<IndustrialManagementTargetChild> listChildData = _repo._context.IndustrialManagementTargetChildren.Where(x => !x.IsDel).Select(x => new IndustrialManagementTargetChild
                {
                    Id = x.Id,
                    IndustrialManagementTargetId = x.IndustrialManagementTargetId,
                    Name = x.Name,
                    Unit = x.Unit
                });
                //IQueryable <IndustrialManagementTargetModel> data = from imt in _repo._context.IndustrialManagementTargets
                //            where !imt.IsDel
                //            group imt by imt.ParentTargetId into g
                //            select new IndustrialManagementTargetModel()
                //            {
                //                ParentTargetId = g.Key,
                //                listChild = g.Select( x => new IndustrialManagementTargetChildModel
                //                {
                //                    IndustrialManagementTargetId = x.IndustrialManagementTargetId,
                //                    Name = x.Name,
                //                    Unit = x.Unit,
                //                    getChild = listChildData.Where(d => d.IndustrialManagementTargetId == x.IndustrialManagementTargetId).Select(d => new ChildModel
                //                    {
                //                        Id = d.Id,
                //                        Name = d.Name,
                //                        Unit = d.Unit
                ...
                IQueryable<ReponseModel> _data = (from imt in _repo._context.IndustrialManagementTargets
                                                  where !imt.IsDel
                                                  join cate in _repo._context.Categories
                                                  on imt.ParentTargetId equals cate.CategoryId
                                                  select new ReponseModel()
                                                  {
                ...
                if (query.SearchValue != null && query.SearchValue != "") //Kiểm tra điều kiện tìm kiếm
                {
                    ...
                    || x.ParentName.ToLower().Contains(_keywordSearch)
        ...
        [HttpPost()]
        public async Task<IActionResult> create(IndustrialManagementTargetModel data)
        {
            BaseModels<IndustrialManagementTargetModel> model = new BaseModels<IndustrialManagementTargetModel>();
            try
            {
                UserModel loginData = Ulities.GetUserByHeader(HttpContext.Request.Headers);
                if (loginData == null)
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(IndustrialManagementTargetModel data)
        {
            BaseModels<IndustrialManagementTargetModel> model = new BaseModels<IndustrialManagementTargetModel>();
            try
            {
                ...
                var CheckData = _repo.FindById(data.IndustrialManagementTargetId);
                SystemLog datalog = new SystemLog();
                if (CheckData == null)
                {
        ...
        {
            BaseModels<IndustrialManagementTargetChildModel> model = new BaseModels<IndustrialManagementTargetChildModel>();
            try
            {
